...
### Added
- Optional `todayColor` parameter on `WeeklyCalendarView` (default `nil`, falls back to `Color.accentColor`). Overrides the background of the circle marking today's date, without affecting the solid-filled selected-date circle or the event indicator dot, which stay `accentColor`.
